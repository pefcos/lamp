...
void print_end_message(int execution_end, char *word, int line)
{
    ...
    case ERROR_NO_SWITCH_FOUND:
        if (word == NULL)
            printf("ERROR: Trying to access an unknown switch on line %d.\n",line);
        else
            printf("ERROR: Trying to access switch \"%s\" on line %d, but it does not exist.\n",word,line);
        break;

    case ERROR_INVALID_REDUCED_NOTATION:
        if (word == NULL)
            printf("ERROR: Invalid reduced switch notation on line %d.\n",line);
        else
            printf("ERROR: Invalid reduced switch notation in \"%s\" on line %d.\n",word,line);
        break;
