...
void print_end_message(int execution_end, char *word)
{
    ...
    case ERROR_NO_SWITCH_FOUND:
        if (word == NULL)
            printf("ERROR: Trying to access an unknown switch.\n");
        else
            printf("ERROR: Trying to access switch \"%s\", but it does not exist.\n",word);
        break;

    case ERROR_INVALID_REDUCED_NOTATION:
        if (word == NULL)
            printf("ERROR: Invalid reduced switch notation.\n");
        else
            printf("ERROR: Invalid reduced switch notation in \"%s\".\n",word);
        break;
